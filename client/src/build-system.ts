...
export async function checkForLocalStoryFormat(
  ...
  if (format instanceof Error) {
    ...
    const outUri = vscode.Uri.joinPath(
      workspaceProvider.rootWorkspaceUri(),
      storyFormatToWorkspacePath(storyFormat, workspaceProvider) ?? "format.js",
    );
    try {
...
export async function build(
  ...
  try {
    ...
    if (
      ...
      story.storyData.storyFormat.format !== cachedStoryFormat.format.format ||
      story.storyData.storyFormat.formatVersion !==
        cachedStoryFormat.format.formatVersion
    ) {
      const maybeStoryFormatData = await readLocalStoryFormatOrAskToDownload(
    ...
    if (includeDir) {
      ...
      for (const includeFileUri of await workspaceProvider.findFiles(
        ...
        if (ndx !== -1) {
          ...
          includeFilepath = includeFileStr.split(includeDir, 1).slice(-1)[0];
        }
