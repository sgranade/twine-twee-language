	...
	describe("Passages", () => {
		describe("General", () => {
			...
			it("should call back with the passage's metadata captured", () => {
				...
				expect(callbacks.passages[0].metadata).to.eql({"position": "600,400", "size": "100,200"});
				...
				it("should call back with both the passage's tags and metadata", () => {
					...
					expect(callbacks.passages[0].metadata).to.eql({"position": "600,400"});
				});
			});
		});
		...
		describe("Special Passages", () => {
			...
			it("should call back on StoryData with the IFID included", () => {
				...
				expect(callbacks.storyData?.ifid).to.eql("62891577-8D8E-496F-B46C-9FF0194C0EAC");
			});
			...
			it("should call back on StoryData with the format included", () => {
				...
				expect(callbacks.storyData?.format).to.eql("MySuperSweetFormat");
			});
		...
		describe("Passages", () => {
			describe("General", () => {
				it("should flag passages with metadata before tags", () => {
					...
					expect(callbacks.errors[0].message).to.include("Tags need to come before metadata");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 36, 0, 41));
				});
				...
				it("should flag passages with text after metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include("Passage headers can't have text after metadata");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 36, 0, 40));
				});
				...
				it("should flag passages with text after tags when there is no metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include("Passage headers can't have text after tags");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 20, 0, 24));
				});
				...
				it("should flag passages with names containing unescaped link markup metacharacters", () => {
					...
					expect(callbacks.errors[0].message).to.include("Passage names can't include ] without a \\ in front of it");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 11, 0, 12));
					expect(callbacks.errors[1].message).to.include("Passage names can't include } without a \\ in front of it");
					expect(callbacks.errors[1].range).to.eql(Range.create(0, 15, 0, 16));
				});
				...
				it("should flag passages with badly-formatted metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include("Metadata isn't properly-formatted JSON.");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 13, 0, 21));
				});
				...
				it("should tell users if their badly-formatted metadata includes a single quote mark", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include("Did you use ' instead of \"?");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 13, 0, 35));
				});
				...
				it("should flag passages with unsupported metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include('Unsupported metadata property');
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 15, 0, 21));
				});
				...
				it("should flag passages with non-string position metadata", () => {
					...
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 25, 0, 27));
				});
				...
				it("should flag passages with incorrectly formatted position metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include('"position" metadata should give the tile location in x,y: "600,400"');
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 26, 0, 29));
				});
				...
				it("should flag passages with non-string size metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include('Must be a string');
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 21, 0, 23));
				});
				...
				it("should flag passages with incorrectly formatted size metadata", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include('"size" metadata should give the tile size in width,height: "100,200"');
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 22, 0, 25));
				});
				...
				it("should flag passages with badly-formatted tags", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include("Tags aren't formatted correctly.");
					expect(callbacks.errors[0].range).to.eql(Range.create(0, 13, 0, 20));
				});
			...
			describe("Special Passages", () => {
				...
				it("should flag StoryData with a missing IFID", () => {
					...
					expect(callbacks.errors.length).to.equal(1);
					expect(callbacks.errors[0].message).to.include('StoryData must include an "ifid" property');
					expect(callbacks.errors[0].range).to.eql(Range.create(1, 0, 2, 1));
				});
