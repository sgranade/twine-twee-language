    ...
    describe("widget", () => {
      it("should capture a macro definition symbol", () => {
        ...
        expect(result).to.eql([
          {
            contents: "testy",
            location: Location.create("fake-uri", Range.create(1, 1, 1, 6)),
...
nope:
    macros:
        ...
        expect(result.errors[0]).to.include("No `sugarcube-2` key found");
      });
...
`.trimStart();
        ...
        const result = uut.tweeConfigFileToMacrosAndEnums(yaml, true).errors;
...
`.trimStart();
        ...
        expect(result[0]).to.include(["values aren't strings: two, three"]);
      });
