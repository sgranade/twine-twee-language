  ...
  describe("Passages", () => {
    it("should warn on repeated passage names in the same document", async () => {
      ...
      const passages = [
        buildPassage({
          label: "Passage 1a",
          location: Location.create("test-uri", Range.create(1, 1, 2, 3)),
        }),
        buildPassage({ label: "Passage 1b" }),
        buildPassage({
          label: "Passage 1a",
          location: Location.create("test-uri", Range.create(4, 4, 5, 5)),
        }),
      ];
      const index = new Index();
    ...
    it("should warn on repeated passage names across documents", async () => {
      ...
      const passages1 = [
        ...
        buildPassage({
          label: "Passage 1b",
          location: Location.create("uri-one", Range.create(1, 1, 2, 3)),
        }),
      ];
      const passages2 = [
      ...
      index.setReferences("test-uri", [
        {
          ...
          locations: [Location.create("test-uri", Range.create(1, 2, 3, 4))],
