      ...
      for (const uri of tweeFileUris) {
        ...
        if (doc !== undefined) {
          ...
          if (
            ...
            newStoryFormat?.formatVersion !== oldStoryFormat?.formatVersion
          ) {
            break;
          }
        }
...
connection.onPrepareRename((params: PrepareRenameParams): Range | undefined => {
  return prepareRename(params.textDocument.uri, params.position, projectIndex);
});
...
async function parseT3LTMacroFile(uri: string) {
  ...
  if (doc !== undefined) {
    const parsedResults = tweeConfigFileToMacrosAndEnums(doc.getText(), isYaml);
