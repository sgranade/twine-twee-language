import { Range, Location, Diagnostic, DiagnosticSeverity } from 'vscode-languageserver';
 ...
 */
function logErrorFor(text: string, at: number, message: string, state: ParsingState): void {
 ...
 * @param state Parsing state.
 */
function logWarningFor(text: string, at: number, message: string, state: ParsingState): void {
	state.callbacks.onParseError(
		createDiagnostic(
	...
	for (const [k, v] of Object.entries(metadataObject)) {
		...
		if (k === 'position') {
			if (typeof v === 'string') {
				...
				logErrorFor(
					vAsString, metadataIndex + valueIndex, `Must be a string.`, state
				);
			}
		}
		else if (k === 'size') {
			...
			logErrorFor(
				k, metadataIndex + keyIndex, `Unsupported metadata property.`, state
			);
		}
	}
 ...
 */
function validateIfid(ifid: unknown, index: number, state: ParsingState): boolean {
	let valid = false;

	if (typeof ifid === 'string') {
		if (!/^[a-fA-F\d]{8}-[a-fA-F\d]{4}-4[a-fA-F\d]{3}-[a-fA-F\d]{4}-[a-fA-F\d]{12}$/.test(ifid)) {
			logErrorFor(ifid, index, `"ifid" must be a version 4 UUID.`, state);
		}
		else {
			valid = true;
			if (/[a-f]/.test(ifid)) {
				logWarningFor(ifid, index, `"ifid" must only have captial letters.`, state);
			}
		}
	}
	else {
...
function parseTagColors(tagColorsObject: any, rawTagColorText: string, rawTagColorIndex: number, state: ParsingState): Map<string, string> | undefined {
 ...
 * @param state Parsing state.
 */
function parseStoryDataPassage(passageText: string, textIndex: number, state: ParsingState): void {
	let storyDataObject;
	let ifidFound = false;
	...
	for (const [k, v] of Object.entries(storyDataObject)) {
		...
		if (k === 'ifid') {
			...
			if (m !== null) {
				const tagColors = parseTagColors(v, m[0], textIndex + m.index, state);
			...
			logErrorFor(
				k, textIndex + keyIndex, `Unsupported StoryData property.`, state
			);
		}
	}
	...
	if (!ifidFound) {
		logErrorFor(
			passageText, textIndex, 'StoryData must include an "ifid" property.', state
		);
	}
...
function parseTwee3(text: string, state: ParsingState): void {
	...
	for (const [passage1, passage2] of pairwise(passages)) {
		const passageTextIndex = state.textDocument.offsetAt(passage1.location.range.end) + 1; // +1 to swallow the \n
	...
	if (lastPassage !== undefined) {
		const passageTextIndex = state.textDocument.offsetAt(lastPassage.location.range.end) + 1; // +1 to swallow the \n
