    ...
    raw: {
      ...
      location: Location.create(
        ...
        Range.create(
          ...
          state.textDocument.positionAt(metadataIndex + rawMetadata.length),
        ),
      ),
    },
  };
          ...
          return {
            ...
            location: Location.create(
              ...
              Range.create(
                ...
                state.textDocument.positionAt(tagIndex + tag.length),
              ),
            ),
          };
        });
      ...
      if (kid.valueNode?.type === "string") {
        ...
        for (const prop of kid.valueNode.properties) {
          if (prop.valueNode?.type === "string") {
            storyData.tagColors[prop.keyNode.value] = prop.valueNode.value;
          }
        }
      }
    }
...
function parsePassageText(
  ...
  if (passage.name.contents === "StoryTitle") {
    ...
    state.storyFormatParser?.parsePassageText(passageText, textIndex, state);
  }
}
...
export function parse(
  ...
  for (const storyDataMatch of docText.matchAll(
    ...
    if (storyDataMatch !== null) {
      const contentStartIndex = storyDataMatch.index + storyDataMatch[0].length;
      ...
      const storyDataText = docText.slice(contentStartIndex, nextPassageIndex);
      ...
      if (
        ...
        storyData.storyFormat?.formatVersion !== storyFormat?.formatVersion
      ) {
