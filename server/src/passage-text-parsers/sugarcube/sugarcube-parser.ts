  ...
  for (const m of passageText.matchAll(customStyleRegex)) {
    ...
    while (
      ...
      if (cssMatch[1] !== undefined) {
        ...
        for (const idOrClassMatch of cssMatch[6].matchAll(singleCssIdOrClass)) {
          capturePreSemanticTokenFor(
...
function parseMacroArgs(
  ...
  if (macroInfo?.parsedArguments !== undefined) {
    ...
    for (let [t3ltArgNdxStr, t3ltArgFormatType] of Object.entries(
      ...
      if (
        ...
        if (passageName !== undefined) {
          ...
          parsePassageReference(passageName, passageAt, state, sugarcubeState);
        }
    ...
    for (const error of validationInfo.info.errors) {
      const errorToken = sc2ArgumentTokens[t3ltArgToSC2Token[error.index] ?? 0];
    ...
    for (const warning of validationInfo.info.warnings) {
      ...
      if (warningToken === undefined) {
        ...
        logErrorFor(warningToken.text, warningToken.at, warning.warning, state);
      }
    }
  ...
  for (const arg of sc2ArgumentTokens) {
    if (arg.type === MacroParse.Item.Error) {
      ...
      props = props.filter((p) => p.prefix && startIsVarRegexp.test(p.prefix));
  ...
  if (macroInfo?.arguments === true) {
    if (args === undefined || !args.trim()) {
      logWarningFor(macroName, macroNameIndex, "Expected arguments", state);
    }
...
function captureMacroRefAndTokens(
  ...
  const deprecated =
    ...
    versionCompare(state.storyFormat.formatVersion, macroInfo.deprecated) <= 0;
  capturePreSemanticTokenFor(
    macroNameWithEnd,
    macroIndex,
    ETokenType.function,
          ...
          for (let i = unclosedMacros.length - 1; i >= 0; --i) {
            if (unclosedMacros[i].name === macroInfo.name) {
              ...
              if (macroInfo.parseChildren !== undefined) {
                const children = macroChildren[unclosedMacros[i].id] ?? [];
                // Shift the kids' locations to be relative to
                // the document instead of the passage text
                for (const child of children) {
                  child.at += textIndex;
                }
                macroInfo.parseChildren(children, state, sugarcubeState);
              }
      ...
      if (macroInfo.parents) {
        ...
        if (parentMacroInfo !== undefined) {
          ...
          const macroParent = macroInfo.parents.find(
            (p) => MacroParent.is(p) && p.name === parentMacroInfo.name,
          );
          if (MacroParent.is(macroParent)) {
            // Make sure we don't have too many of the same kind of child macro
            const childCount = macroChildren[parentMacroInfo.id].filter(
              (info) => info.name === macroInfo.name,
            ).length;
...
function parseHtmlAttributesAndDirectives(
  ...
  for (const m of passageText.matchAll(htmlTagRegex)) {
    ...
    for (const m1 of tag.matchAll(sc2AttrsAndDirectives)) {
      ...
      let attrContentsIndex = attrIndex + m1[0].length - attrContents.length;
      ...
      if (attrName === "data-passage") {
        ...
        if (evalDirective !== undefined && attrName.endsWith("data-setter")) {
          logErrorFor(
            evalDirective,
            textIndex + attrIndex,
...
export function parsePassageText(
  ...
  // is a script passage
  if (state.parseLevel !== ParseLevel.Full || state.currentPassage?.isScript) {
    return;
  }
