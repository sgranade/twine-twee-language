...
const parameterTypes: ParameterType[] = [
  ...
  makeSimpleParameterType(
    ...
    (type) => type === ArgType.SettingsSetupAccess || type === ArgType.Variable,
  ),
  {
    name: ["receiver"],
    validate(info: ArgumentInfo): ValidateResult {
      if (info.arg.type === ArgType.String) {
        ...
        return ["Argument is not a potentially valid variable receiver", true];
      }
    },
  },
  {
    ...
    validate(info: ArgumentInfo): ValidateResult {
      ...
      if (info.arg.setter) {
        return ["Argument is a link, but does not allow setter syntax", false];
      }
...
class Variant {
  ...
  validate(args: Arg[]): ValidateInformation {
    ...
    function crawl(format: Format, argIndex: number): CrawlInformation {
      ...
      if (format.kind === FormatKind.MaybeNext) {
        ...
        const infoRight = crawl(format.right, argIndex);
        if (!isFailure(infoRight.status)) {
          Object.assign(infoRight.argFormatTypes, infoLeft.argFormatTypes);
          return infoRight;
        }
        ...
        const success = makeSuccess(argIndex + 1, argFormatTypes, correctRank);
        ...
        if (arg === undefined) {
          ...
          return makeSuccess(argIndex + 1, argFormatTypes, correctRank);
        }
  ...
  private static parseFormat(formatString: string): Format | null {
    ...
    function prefixBindingPower(op: FormatKind): number {
      switch (op) {
        ...
        default:
          throw new Error(
            "Parse: Unhandled operator in infix binding power of: " + op,
          );
      }
    }
    ...
    function isPrefixOperator(
      ...
      return (
        token.kind === FormatKind.MaybeNext || token.kind === FormatKind.Repeat
      );
    }

    function isValue(token: FormatLex): token is FormatLiteral | FormatType {
      return (
        token.kind === FormatKind.Literal || token.kind === FormatKind.Type
      );
    }
  ...
  private static lexFormat(formatString: string): FormatLex[] {
    ...
    function makeRange(start: number, end: number, line: number = 0): Range {
