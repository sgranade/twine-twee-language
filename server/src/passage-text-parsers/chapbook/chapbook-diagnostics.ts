...
function generateCustomInsertDiagnostics(
  ...
  for (const insertRef of index.getReferences(
    ...
    const insert = customInserts.find((i) => i.match.test(insertRef.contents));
    if (insert !== undefined) {
...
function generateCustomModifierDiagnostics(
  ...
  for (const modRef of index.getReferences(
    ...
    const modifier = customModifiers.find((i) => i.match.test(modRef.contents));
...
export function generateDiagnostics(
  ...
  const varNamesWithDuplicates = lookupVariables.map((x) => x.split(".", 1)[0]);
  for (const uri of index.getIndexedUris()) {
